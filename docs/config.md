...
There is no standard wat of attaching a logo to an OCI image. As such, Cupdate
...
If you want to have logos for other images you can put them in the directory
controlled by the `CUPDATE_LOGOS_PATH` environment variable (defaults to
`logos`). Cupdate will then look for logos that match the image's name. For
example, place a logo for `ghcr.io/alexgustafsson/cupdate` in the
`ghcr.io/alexgustafsson` directory and name the image `cupdate.png`.
